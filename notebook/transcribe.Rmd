...
# Not yet finnished
# Cant overwrite
def convert_audio_2(in_file, out_file):
    process = (
        ffmpeg
        .input(in_file)
        .output(out_file)
...
```{python}
convert_audio('../uploaded_file/Sampel1_dari_librosa.wav', '../uploaded_file/Sampel1_fmpeg_default.wav')
```
...
def test_band_pass():
    ...
    audio, sr = librosa.load("uploaded_file/Sampel22.wav", sr=None)
    
    filtered_audio = butter_bandpass_filter(audio, lowcut, highcut, sr)
    librosa.output.write_wav("uploaded_file/Sampel222.wav", filtered_audio, sr, norm=False)
